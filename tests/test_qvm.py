...
class TestQVMBasic(BaseTest):
    ...
    def test_identity_expectation(self, shots, qvm, compiler, parallel):
        """Test that identity expectation value (i.e. the trace) is 1"""
        theta = 0.432
        phi = 0.123

        dev = plf.QVMDevice(device="2q-qvm", shots=shots, parallel=parallel)
    ...
    @pytest.mark.parametrize("parallel", [False, True])
    def test_pauliz_expectation(self, shots, qvm, compiler, parallel):
    ...
    @pytest.mark.parametrize("parallel", [False, True])
    def test_paulix_expectation(self, shots, qvm, compiler, parallel):
    ...
    @pytest.mark.parametrize("parallel", [False, True])
    def test_pauliy_expectation(self, shots, qvm, compiler, parallel):
    ...
    @pytest.mark.parametrize("parallel", [False, True])
    def test_hadamard_expectation(self, shots, qvm, compiler, parallel):
    ...
    def test_hermitian_expectation(self, shots, qvm, compiler):
        ...
        dev = plf.QVMDevice(device="2q-qvm", shots=shots, parallel=True)
    ...
    def test_multi_qubit_hermitian_expectation(
        self, shots, execution_timeout, qvm, compiler, parallel
    ):
    ...
    @pytest.mark.parametrize("parallel", [False, True])
    def test_var(self, shots, qvm, compiler, parallel):
    ...
    @pytest.mark.parametrize("parallel", [False, True])
    def test_var_hermitian(self, shots, execution_timeout, qvm, compiler, parallel):
    ...
    def test_apply(self, op, apply_unitary, shots, qvm, compiler):
        ...
        dev = plf.QVMDevice(
            device="3q-qvm", shots=shots, parametric_compilation=False, parallel=True
        )
    ...
    @pytest.mark.parametrize("parallel", [False, True])
    def test_sample_values(self, qvm, tol, parallel):
        """Tests if the samples returned by sample have
        the correct values
        """
        dev = plf.QVMDevice(device="1q-qvm", shots=10, parallel=parallel)
    ...
    @pytest.mark.parametrize("parallel", [False, True])
    def test_sample_values_hermitian(self, qvm, execution_timeout, tol, parallel):
    ...
    def test_error_if_parallel_with_no_threads(self):
        """Test that an error is raised if the parallel option is set to True but max_threads
        is 0 or less."""
        with pytest.raises(
            ValueError,
            match="max_threads must be set to a positive integer greater than 0",
        ):
            plf.QVMDevice(device="2q-qvm", shots=5, parallel=True, max_threads=0)
...
class TestParametricCompilation(BaseTest):
    ...
    def test_parametric_compilation_with_numeric_and_symbolic_queue(
        self, mock_qvm, execution_timeout, parallel
    ):
...
class TestQVMIntegration(BaseTest):
    ...
    def test_qubit_unitary(
        self, shots, compiler_timeout, execution_timeout, qvm, compiler, parallel
    ):
    ...
    @pytest.mark.parametrize("device", ["2q-qvm", np.random.choice(TEST_QPU_LATTICES)])
    @pytest.mark.parametrize("parallel", [False, True])
    def test_2q_gate(self, device, qvm, compiler, parallel):
...
class TestQVMParallelExecution:
    ...
    def test_parallel_processing_without_parametric_compilation(self):
        dev = qml.device(
            "rigetti.qvm",
            device="2q-qvm",
            shots=QVM_SHOTS,
